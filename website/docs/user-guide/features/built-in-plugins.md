...
Adds deterministic identifiers and local operational state for prompt-prefix and KV-cache reuse without coupling the Hermes agent loop to a specific inference engine. It uses LLM request middleware to attach an authoritative cache manifest and LLM execution middleware to record duration, success/failure, and provider-reported cache-read/write tokens.
...
By default, cache identifiers are sent only to loopback, private/link-local, LAN, and Tailscale endpoints. A public URL does not become trusted because its provider is labeled `lmstudio` or `ollama`. Trusted remote cache proxies require explicit `HERMES_CACHE_ALLOW_REMOTE=1` opt-in.
