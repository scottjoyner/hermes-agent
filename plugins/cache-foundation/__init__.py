...
def _cache_request(**kwargs: Any) -> dict[str, Any] | None:
    ...
    if not isinstance(request, dict):
        return None
    if str(kwargs.get("api_mode") or "") != "chat_completions":
        return None

    base_url = str(kwargs.get("base_url") or "")
    if not _eligible_endpoint(base_url):
        return None
...
def _usage_metrics(response: Any) -> dict[str, int]:
    ...
    prompt_tokens = int(
        _read_attr(usage, "prompt_tokens", 0)
        or _read_attr(usage, "input_tokens", 0)
        or 0
    )
    ...
    if input_details is not None:
        cache_read = max(
            cache_read,
            int(_read_attr(input_details, "cached_tokens", 0) or 0),
        )
...
def _cache_execution(**kwargs: Any) -> Any:
    """Measure one real provider call while preserving exactly-once behavior."""
    ...
    if str(kwargs.get("api_mode") or "") != "chat_completions":
        return next_call(request)
    if not _eligible_endpoint(str(kwargs.get("base_url") or "")):
        return next_call(request)
    ...
    try:
        ...
        try:
            ...
            print(
                f"cache-foundation: telemetry write failed: {store_exc}",
                file=sys.stderr,
            )
        raise
...
def _setup_cli(parser: argparse.ArgumentParser) -> None:
    ...
    checkpoints = commands.add_parser(
        "checkpoints",
        help="List known checkpoint inventory",
    )
    ...
    doctor = commands.add_parser(
        "doctor",
        help="Inspect cache configuration and endpoint type",
    )
    ...
    warm = commands.add_parser(
        "warm",
        help="Warm an OpenAI-compatible local prefix",
    )
    ...
    _print_json(
        {
            ...
            "requests": _store().recent_requests(
...
def _cmd_doctor(endpoint: str, provider: str, timeout: float) -> int:
    ...
    payload: dict[str, Any] = {
        ...
        "enabled": not _disabled(),
        "endpoint": endpoint,
        "eligible": _eligible_endpoint(endpoint) if endpoint else None,
        "provider": provider,
        "engine": engine,
...
def _cmd_warm(endpoint: str, model: str, prompt_file: str, timeout: float) -> int:
    ...
    if not path.is_file():
        ...
        return 2
    if not _eligible_endpoint(endpoint):
        print(
